...
import { View, Text, Modal, ScrollView, Pressable, ActivityIndicator, KeyboardAvoidingView, Platform, StyleSheet } from 'react-native';
...
export const CheckoutModal: React.FC<CheckoutModalProps> = ({
    ...
    const mixedPaymentDetails = (() => {
        ...
        const total = selectedPedido.total + (agregarPropina ? (selectedPedido.total * 0.10) : 0);
            ...
            <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.modalOverlay}>
                <View style={[styles.checkoutModal, { backgroundColor: cardBg, borderColor }]}>
                    ...
                    <ScrollView style={{ flexShrink: 1 }} contentContainerStyle={{ paddingBottom: 16 }} showsVerticalScrollIndicator={false}>
                        ...
                        {loadingDetails ? (
                            <View style={[styles.orderMetaRow, { borderColor, padding: 12, gap: 10 }]}>
                                <Skeleton width={60} height={40} borderRadius={8} style={{ flex: 1 }} />
                        ...
                        <View style={styles.paymentMethodsGrid}>
                            {['efectivo', 'tarjeta', 'transferencia', ...(selectedClient ? ['prepago'] : [])].map((m: any) => {
                                ...
                                const isLockedPrepago = m !== 'prepago' && (selectedClient?.saldo || 0) > 0;
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        />
                                        <Text style={[styles.payMethodLabel, { color: isSelected ? accentColor : textSecondary, textTransform: 'capitalize' }]}>{m}</Text>
                                    </Pressable>
                                );
                            })}
                        </View>
                        ...
                        {selectedClient && (
                            <View style={{ marginTop: 10, padding: 12, borderRadius: 12, backgroundColor: accentBg, borderWidth: 1, borderColor: accentBorder }}>
                                <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                                    <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                                        <Ionicons name="person-circle" size={20} color={accentColor} />
                                        <Text style={{ color: textPrimary, marginLeft: 8, fontWeight: '700' }}>{selectedClient.nombre} {selectedClient.apellido}</Text>
                                    </View>
                                    <View>
                                        <Text style={{ color: textSecondary, fontSize: 11, fontWeight: '800' }}>SALDO PREPAGO</Text>
                                        <Text style={{ color: Number(selectedClient.saldo) > 0 ? '#10B981' : textSecondary, fontSize: 15, fontWeight: '900', textAlign: 'right' }}>
                        ...
                        {mixedPaymentDetails.isMixed && (
                            <View style={{ marginTop: 15, padding: 15, borderRadius: 16, borderStyle: 'dashed', borderWidth: 1, borderColor: accentColor }}>
                                <Text style={{ color: textPrimary, fontSize: 13, fontWeight: '800', marginBottom: 10 }}>PAGO MIXTO REQUERIDO</Text>
                                <View style={styles.summaryRow}>
                                    <Text style={{ color: textSecondary, fontSize: 13 }}>Saldo Prepago usado:</Text>
                                    <Text style={{ color: '#10B981', fontWeight: '800' }}>-${mixedPaymentDetails.prepago.toLocaleString()}</Text>
                                </View>
                                <View style={[styles.summaryRow, { marginTop: 4 }]}>
                                    <Text style={{ color: textSecondary, fontSize: 13 }}>Restante a pagar:</Text>
                                    <Text style={{ color: textPrimary, fontWeight: '800' }}>${mixedPaymentDetails.adicional.toLocaleString()}</Text>
                                </View>
                                
                                <View style={{ marginTop: 12 }}>
                                    <Text style={{ color: textPrimary, fontSize: 11, fontWeight: '900', marginBottom: 8, letterSpacing: 0.5 }}>MÉTODO PARA EL RESTANTE</Text>
                                            ...
                                            return (
                                                <Pressable
                                                    ...
                                                    style={{
                                                        ...
                                                        backgroundColor: isSel ? `${accentColor}15` : 'transparent',
                                                        alignItems: 'center'
                                                    }}
                                                >
                                                    <Text style={{ color: isSel ? accentColor : textSecondary, fontSize: 10, fontWeight: '800' }}>{m.toUpperCase()}</Text>
                                                </Pressable>
                                            );
                                        })}
                                    </View>
