...
export default function AnticiposScreen() {
    ...
    const {
        ...
        fetchAnticipos, fetchMaximo, solicitarAnticipo, onRefresh
    } = useAnticipos();
    ...
    const totalPendiente = solicitudes.filter(a => a.estado === 'pendiente').reduce((sum, a) => sum + Number(a.monto), 0);
    const totalEnCaja = solicitudes.filter(a => a.estado === 'confirmada').reduce((sum, a) => sum + Number(a.monto), 0);
                        ...
                        <View style={[styles.statusBadge, {
                            backgroundColor: isPagoReal ? (isDark ? '#065F4630' : '#D1FAE5') :
                                isPendiente ? (isDark ? '#451A03' : '#FEF3C7') :
                                    isAprobada ? (isDark ? '#065F46' : '#D1FAE5') : (isDark ? '#7C2D12' : '#FEE2E2'),
                        }]}>
                            <Text style={[styles.statusText, {
                                color: isPagoReal ? (isDark ? '#10B981' : '#047857') :
                                    isPendiente ? '#F59E0B' :
                                        isAprobada ? (isDark ? '#6EE7B7' : '#065F46') : (isDark ? '#F87171' : '#B91C1C'),
                            }]}>
    ...
    if (loading) return (
        <View style={[styles.container, { backgroundColor: bg }]}>
            ...
            <View style={{ margin: 16 }}>
                <SkeletonLoader width="100%" height={140} borderRadius={16} /></View>
    ...
    return (
        <View style={[styles.container, { backgroundColor: bg }]}>
            ...
            <View style={[styles.summaryCard, { backgroundColor: cardBg, borderColor }]}>
                ...
                <Text style={[styles.summaryAmount, { color: viewMode === 'solicitudes' ? '#F59E0B' : accentColor }]}>${viewMode === 'solicitudes' ? formatCurrency(totalPendiente + totalEnCaja) : formatCurrency(totalPagado)}</Text>
                <View style={styles.summaryDetails}>
                    <Text style={[styles.summaryDetail, { color: textSecondary }]}>
                        {viewMode === 'solicitudes' ? `Pendientes: $${formatCurrency(totalPendiente)} | Aprobadas: $${formatCurrency(totalEnCaja)}` : `Total retirado históricamente: $${formatCurrency(totalPagado)}`}
                    </Text>
                </View>
            </View>
                ...
                <View style={styles.filterRow}>
                    {(['todos', 'pendiente', 'aprobado', 'rechazado'] as const).map((f) => (
                        <Pressable key={f} style={[styles.filterButton, { backgroundColor: filter === f ? accentColor : cardBg, borderColor: filter === f ? accentColor : borderColor }]} onPress={() => setFilter(f)}>
                            <Text style={[styles.filterText, { color: filter === f ? '#FFFFFF' : textSecondary }]}>{f.charAt(0).toUpperCase() + f.slice(1)}s</Text>
                        </Pressable>
                    ))}
                </View>
            )}
            ...
            <Pressable style={[styles.fab, { backgroundColor: accentColor }]} onPress={openSolicitarModal}><Ionicons name="add" size={28} color="#FFFFFF" /></Pressable>

            <Modal visible={modalVisible} animationType="slide" transparent onRequestClose={() => setModalVisible(false)}>
                <View style={styles.modalOverlay}>
                    <View style={[styles.modalContent, { backgroundColor: bg }]}>
                        <View style={styles.modalHeader}><Text style={[styles.modalTitle, { color: textPrimary }]}>Solicitar Anticipo</Text><Pressable onPress={() => setModalVisible(false)}><Ionicons name="close" size={24} color={textSecondary} /></Pressable></View>
                        <View style={[styles.disponibleCard, { backgroundColor: cardBg, borderColor }]}>
                            ...
                            <View style={styles.desgloseRow}>
                                {[{ label: 'Asistencia', value: montoAsistencia }, { label: 'Comisiones', value: montoComisiones }, { label: 'Propinas', value: montoPropinas }].map(({ label, value }) => (
                                    <View key={label} style={styles.desgloseItem}><Text style={[styles.desgloseLabel, { color: textSecondary }]}>{label}</Text><Text style={[styles.desgloseValue, { color: textPrimary }]}>${formatCurrency(value)}</Text></View>
                                ))}
                            </View>
                        </View>
                        <TextInput style={[styles.input, { backgroundColor: cardBg, color: textPrimary, borderColor }]} placeholder={`Monto (máx. $${formatCurrency(montoMaximo)})`} placeholderTextColor={textSecondary} keyboardType="numeric" value={monto} onChangeText={handleMontoChange} />
                        <TextInput style={[styles.input, styles.textArea, { backgroundColor: cardBg, color: textPrimary, borderColor }]} placeholder="Motivo (opcional)" placeholderTextColor={textSecondary} multiline numberOfLines={3} value={motivo} onChangeText={setMotivo} />
                        <Pressable style={[styles.submitButton, { backgroundColor: accentColor }, (sendingRequest || montoMaximo === 0) && { opacity: 0.7 }]} onPress={handleSolicitar} disabled={sendingRequest || montoMaximo === 0}>
                            {sendingRequest ? <ActivityIndicator color="#FFFFFF" /> : <Text style={styles.submitButtonText}>{montoMaximo === 0 ? 'Sin monto disponible' : 'Enviar Solicitud'}</Text>}
                        </Pressable>
                    </View>
                </View>
            </Modal>
