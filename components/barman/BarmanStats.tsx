...
export const BarmanStats = ({ stats, fullWidth = false }: BarmanStatsProps) => {
    ...
    const weeklyTotal = Array.isArray(stats?.weeklyIncome)
        ? stats.weeklyIncome.reduce((sum: number, value: number) => sum + (value || 0), 0)
        : 0;
    ...
    const statItems = [
        {
            ...
            color: '#10B981',
            isFullWidth: true,
            subLabel: (stats?.svcCount || 0) + ' movimientos'
        },
        {
            label: 'Esta Semana',
