...
const buildCommissionPreview = (items: any[], hostesses: any[]) => {
  const totalCommission = items.reduce(
    (acc, item) =>
      acc + Number(item.comision || 0) * Number(item.cantidad || 0),
    0,
  );
  ...
  const addAmount = (hostessId: string | number, amount: number) => {
    ...
    const hostess = hostesses.find(
      (item: any) => String(item.id_usuario || item.id) === key,
    );
  ...
  items.forEach((item) => {
    ...
    const itemCommission =
      Number(item.comision || 0) * Number(item.cantidad || 0);
  ...
  return {
    totalCommission,
    assignedCommission: Array.from(distribution.values()).reduce(
      (acc, item) => acc + item.amount,
      0,
    ),
    hostessDistribution: Array.from(distribution.values()).sort(
        ...
        {cart.length > 0 && (
          <CartList
            ...
            onUpdateQuantity={(idx: number, delta: number) => {
              const newCart = [...cart];
              newCart[idx].cantidad = Math.max(
                1,
                newCart[idx].cantidad + delta,
              );
              newCart[idx].subtotal =
                newCart[idx].precio * newCart[idx].cantidad;
              dispatch({ type: "SET_CART", payload: newCart });
            }}
            onRemove={(idx: number) => {
              const newCart = [...cart];
      ...
      <NuevaCuentaModales
        ...
        onToggleHostess={(id) => {
          ...
          if (currentSelected.some((x) => String(x) === strId)) {
            ...
            if (
              hostessSelectionTarget.max &&
              currentSelected.length >= hostessSelectionTarget.max
            ) {
              showToast(
                "Límite",
                `Máximo ${hostessSelectionTarget.max} anfitrionas por esta cantidad`,
                "error",
              );
              return;
            }
        ...
        onConfirmHostess={() => {
          if (hostessSelectionTarget?.product) {
            ...
            if (hasComm && currentSelected.length === 0) {
              showToast(
                "Asignación",
                "Debes escoger al menos 1 anfitriona",
                "error",
              );
              return;
            }
        ...
        onSetQuantity={(productId, qty) =>
          dispatch({ type: "SET_MODAL_QUANTITY", productId, quantity: qty })
        }
        onSetHostessTarget={(target) =>
          dispatch({ type: "SET_HOSTESS_TARGET", target })
        }
