                            ...
                            <ScrollView showsVerticalScrollIndicator={false}>
                                ...
                                <View style={[styles.divisionCard, { borderColor, backgroundColor: isDark ? 'rgba(255,255,255,0.03)' : 'rgba(0,0,0,0.02)' }]}>
                                    <View style={styles.divisionItem}>
                                        <Text style={[styles.divisionLabel, { color: textSecondary }]}>Propina Total</Text>
                                        <Text style={[styles.divisionValue, { color: textPrimary }]}>
                                            ${(parentPropina?.monto || saleDetail?.propina || 0).toLocaleString()}
                                        </Text>
                                    </View>
                                    ...
                                    <View style={styles.divisionItem}>
                                        ...
                                        <Text style={[styles.divisionValue, { color: accentColor }]}>
                                            {parentPropina?.conteo_usuarios || parentPropina?.cantidad_personas || (parentPropina?.detalles?.length) || '---'}
                                        </Text>
                                    </View>
                                    <View style={styles.divisionDivider} />
                                    <View style={styles.divisionItem}>
                                ...
                                {saleDetail ? (
                                    <>
                                        <Text style={[styles.sectionTitle, { color: textSecondary, marginTop: 20 }]}>PRODUCTOS DE LA VENTA</Text>
                                        <View style={[styles.productsList, { borderColor }]}>
                                            {saleDetail.detalles?.map((det: any, idx: number) => (
                                                <View key={idx} style={[styles.productRow, { borderBottomColor: borderColor, borderBottomWidth: idx === saleDetail.detalles.length - 1 ? 0 : 1 }]}>
                                                    <View style={{ flex: 1 }}>
                                                        ...
                                                        <Text style={{ color: textSecondary, fontSize: 12 }}>{det.cantidad} x ${det.precio.toLocaleString()}</Text>
                                                    </View>
                                                    <Text style={[styles.productSubtotal, { color: textPrimary }]}>${det.sub_total.toLocaleString()}</Text>
                                                </View>
                                            ))}
                                            <View style={[styles.saleTotalRow, { borderTopColor: borderColor }]}>
                                                ...
                                                <Text style={[styles.saleTotalValue, { color: textPrimary }]}>${saleDetail.total.toLocaleString()}</Text>
                                            </View>
                                        </View>

                                        <View style={{ marginTop: 15, padding: 12, borderRadius: 12, backgroundColor: isDark ? '#37415140' : '#F3F4F6' }}>
                                            <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 5 }}>
                                                <Text style={{ color: textSecondary, fontSize: 12 }}>Método de Pago</Text>
                                                <Text style={{ color: textPrimary, fontSize: 12, fontWeight: '700' }}>{saleDetail.metodo_pago?.toUpperCase()}</Text>
                                            </View>
                                            <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                                                <Text style={{ color: textSecondary, fontSize: 12 }}>Fecha</Text>
                                                <Text style={{ color: textPrimary, fontSize: 12 }}>{new Date(saleDetail.fecha_crea).toLocaleString()}</Text>
                                            </View>
                                        </View>
                                    </>
                                ) : (
                                    <View style={{ padding: 40, alignItems: 'center' }}>
                                        ...
                                        <Text style={{ color: textSecondary, marginTop: 10, textAlign: 'center' }}>
                                            No se pudieron cargar los detalles de la venta asociada.
                                        </Text>
                                    </View>
                                )}
                            </ScrollView>
