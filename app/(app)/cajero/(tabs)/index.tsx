...
export default function CajeroHomeScreen() {
  ...
  const [state, dispatch] = useReducer(cajeroReducer, {
    ...initialState,
  ...
  const showAlert = useCallback(
    (
      title: string,
      message: string,
      type: CajeroState["alertConfig"]["type"] = "info",
      onConfirm?: () => void,
      showCancel = false,
    ) => {
      dispatch({
  ...
  const fetchData = useCallback(async (isManual = false) => {
    try {
      const [statsRes, userRes, pendingRes, statusRes, eventsRes] =
      ...
      const newData = {
        stats,
        user: userResVal?.user,
        pending: pendingResVal?.count || 0,
        status: statusResVal?.status,
      };
      ...
      es.addEventListener("message", (event: any) => {
        ...
        try {
          ...
          if (
            ...
            console.log(
              `[CajeroHome] Event ${payload.type} received, refreshing stats`,
            );
            fetchData();
          }
        } catch (err) {
        ...
        <AnimatedScreen>
          <LinearGradient
            ...
            style={[
              styles.header,
              {
                paddingTop: insets.top + (isTablet ? 30 : 10),
          ...
          >
            <View style={isTablet && styles.tabletMaxWidth}>
              <PremiumHeaderActions
                ...
                onPersonalPress={() => router.push("/cajero/personal")}
              />
