...
const ServiceCard = memo(
  ({
    ...
    const hostesses = item.anfitrionas
      ? item.anfitrionas.split(",").map((h) => h.trim())
      : [];
      ...
      >
        ...
        <View style={styles.cardHeader}>
          <View style={styles.roomBadge}>
            <View
              style={[styles.iconBox, { backgroundColor: theme.accent + "15" }]}
            >
              <Ionicons name="bed" size={18} color={theme.accent} />
            </View>
            <View>
              <Text style={[styles.roomName, { color: theme.text }]}>
                {item.roomName || "Habitación"}
              </Text>
              <Text style={[styles.serviceCode, { color: theme.textMuted }]}>
                #{item.servicioCode || "S/N"}
              </Text>
            </View>
          </View>
          <View
          ...
          <View style={styles.infoCol}>
            ...
            <View style={styles.valueGroup}>
              ...
              <View style={{ flex: 1 }}>
                ...
                item.clienteNombre !== "Sin cliente registrado" ? (
                  (() => {
        ...
        <View
          style={[
            styles.financeBox,
            { backgroundColor: isDark ? "#FFFFFF05" : "#F1F5F9" },
          ]}
        >
          <View style={styles.financeGrid}>
            ...
            <View style={styles.finItem}>
              ...
              <Text style={[styles.finValue, { color: theme.accent }]}>
                ${iva.toLocaleString()}
              </Text>
            </View>
          </View>
          ...
          <View
            style={[styles.finDivider, { backgroundColor: theme.border }]}
          />
          ...
          <View style={styles.totalRow}>
            <View>
              ...
              <View style={styles.methodTag}>
                ...
                <Text style={[styles.methodText, { color: theme.textMuted }]}>
...
export default function ServiciosActivosScreen() {
  ...
  const [activeTab, setActiveTab] = useState<"activos" | "finalizados">(
    "activos",
  );
  ...
  const fetchFinalizados = async (isManual = false) => {
    ...
    try {
      ...
      console.log(
        "[Servicios] Respuesta caja:",
        JSON.stringify(cajaRes, null, 2),
      );
      ...
      if (cajaRes.success && cajaRes.data?.hasOpenCaja) {
        cajaId =
          cajaRes.data?.cajaInfo?.id_caja ||
          cajaRes.data?.openCaja?.id_caja ||
          cajaRes.data?.caja?.id_caja;
      ...
      console.log(
        "[Servicios] Estructura completa cajaRes:",
        JSON.stringify(cajaRes, null, 2),
      );
  ...
  const onRefresh = async () => {
    ...
    if (activeTab === "activos") {
      ...
      Toast.show({
        ...
        text1: hasChanges ? "Éxito" : "Información",
        text2: hasChanges
          ? "Temporizadores actualizados"
          : "Sin cambios en los servicios activos",
        visibilityTime: 3000,
      });
    } else {
  ...
  const onFinalizar = useCallback(
    (timer: Timer) => {
      setAlertConfig({
        ...
        message: `¿Estás seguro de que deseas finalizar el servicio en la ${timer.roomName}? Si hay una sesión previa pausada en esta habitación, se reanudará automáticamente.`,
        type: "danger",
        onConfirm: async () => {
          setAlertConfig((prev) => ({ ...prev, visible: false }));
          try {
            const res = await apiClient(`/servicios/${timer.servicioId}`, {
              method: "PATCH",
            ...
            if (res.success) {
              Toast.show({
                type: "success",
                text1: "Éxito",
                text2: "El servicio ha sido finalizado correctamente.",
                visibilityTime: 3000,
              });
              refreshTimers();
              ...
              Toast.show({
                type: "error",
                text1: "Error",
                text2: res.message || "No se pudo finalizar el servicio",
                visibilityTime: 4000,
              });
            }
          } catch (error) {
            Toast.show({
              type: "error",
              text1: "Error",
              text2: "Error de conexión al finalizar el servicio",
              visibilityTime: 4000,
            });
          }
        },
    ...
    [refreshTimers, activeTab, fetchFinalizados],
  );
  ...
  const handleEditSuccess = () => {
    ...
    Toast.show({
      ...
      text2: "Los detalles del servicio han sido modificados.",
    });
  };
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.bg }]}>
      ...
      <View
        style={[
          styles.header,
          { backgroundColor: theme.card, paddingTop: insets.top + 10 },
        ]}
      >
        ...
        <View style={styles.tabBar}>
          <Pressable
            style={[
              styles.tabBtn,
              activeTab === "activos" && { borderBottomColor: theme.accent },
            ]}
          ...
          >
            <Text
              style={[
                ...
                {
                  color:
                    activeTab === "activos" ? theme.accent : theme.textMuted,
                },
              ]}
            >
              ACTIVOS
      ...
      {loadingTimers || (activeTab === "finalizados" && loadingFinalizados) ? (
        ...
        <FlatList
          data={
            activeTab === "activos"
              ? timers.filter(
                  (t) => t.tipoTransaccion === "servicio" && t.isActive,
                )
              : finalizados
          }
          renderItem={({ item }) => (
          ...
          ListEmptyComponent={
            <View style={styles.empty}>
              ...
              <Text style={[styles.emptySub, { color: theme.textMuted }]}>
                {activeTab === "activos"
                  ? "Los privados activos aparecerán aquí."
                  : "No tienes servicios finalizados todavía."}
              </Text>
            </View>
          }
        />
