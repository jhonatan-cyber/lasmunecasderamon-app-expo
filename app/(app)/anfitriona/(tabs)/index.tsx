...
function anfitrionaReducer(
  state: AnfitrionaState,
  action: AnfitrionaAction,
): AnfitrionaState {
  switch (action.type) {
    case "SET_LOADING":
...
export default function AnfitrionaHomeScreen() {
  ...
  const showAlert = useCallback(
    (
      title: string,
      message: string,
      type: AnfitrionaState["alertConfig"]["type"] = "info",
      onConfirm?: () => void,
      showCancel = false,
    ) => {
      dispatch({
  ...
  const fetchData = useCallback(async (isManual = false) => {
    try {
      ...
      const activeSvc = serviciosRes.success
        ? serviciosRes.data.find((s: any) => s.estado === 2)
        : null;
  ...
  const dailyGrowth = useMemo(() => {
    ...
    const sorted = [...stats.weeklyIncome].sort(
              ...
              {!activeService && (
                ...
                >
                        ...
                        async () => {
                          try {
                            ...
                            if (res.success) {
                              Toast.show({
                                type: "success",
                                text1: "Llamado enviado",
                                text2: `Personal notificado ${activeService ? `en Hab. ${activeService.habitacion}` : ""}`,
                              });
                            }
                          } catch (err) {
                            Toast.show({
                              type: "error",
                              text1: "Error",
                              text2: "No se pudo enviar el llamado",
                            });
                          }
                        },
                        true,
                  ...
                  >
                    ...
                    <RNText style={styles.callStaffBtnText}>
                      SOLICITAR PERSONAL
                    </RNText>
                  </AnimatedButton>
                </MotiView>
              ...
              >
                ...
                >
                  <View style={styles.activeServiceHeader}>
                    <View>
                      <RNText style={styles.activeServiceLabel}>
                        SERVICIO EN CURSO
                      </RNText>
                  ...
                  <View style={styles.activeServiceActions}>
                    <Pressable
                      ...
                      onPress={() => handleAssistance("Tragos")}
                    >
                    ...
                    <Pressable
                      style={styles.actionIconButton}
                      onPress={() => handleAssistance("Limpieza")}
                    >
                    ...
                    <Pressable
                      style={styles.actionIconButton}
                      onPress={() => handleAssistance("Seguridad")}
                    >
                      <Ionicons
            ...
            >
              <Pressable
                onPress={() => {
                  ...
                  const dailyNeeded =
                    missing > 0
                      ? Math.round(missing / Math.max(1, daysLeft))
                      : 0;
              ...
              >
                ...
                <View style={{ alignItems: "center" }}>
                  <DonutChart
                    percent={Math.min(
                      100,
                      Math.round(((stats?.totalEarnings || 0) / 50000) * 100),
                    )}
                    color={accentColor}
                    isDark={isDark}
                    size={70}
                    strokeWidth={5}
                  />
                  <RNText style={[styles.goalStatus, { color: textSecondary }]}>
                    ${(stats?.totalEarnings || 0).toLocaleString()}{" "}
                    <RNText style={{ opacity: 0.5 }}>/ $50k</RNText>
                  </RNText>
                </View>
              </Pressable>
            </MotiView>
            ...
            >
              <View style={styles.cardHeader}>
                ...
                <Ionicons
                  name="trending-up"
                  size={14}
                  color={dailyGrowth >= 0 ? "#10B981" : "#EF4444"}
                />
              </View>
              <View
                style={{
              ...
              >
                ...
                <RNText style={[styles.statLabel, { color: textSecondary }]}>
                  v/s ayer
                </RNText>
              </View>
            </MotiView>
