...
export const TimerProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const handleSSEEvent = (payload: any) => {
        switch (payload.type) {
            case 'timer_started':
                ...
                setTimers(prev => [...prev.filter(t => t.servicioId !== newTimer.servicioId), newTimer]);
                break;

            case 'timer_stopped':
                setTimers(prev => prev.filter(t => t.servicioId !== payload.data.servicioId));
                break;

            case 'timer_paused':
                setTimers(prev => prev.map(t =>
                    t.servicioId === payload.data.servicioId ? { ...t, isPaused: true, estado: 3 } : t
                ));
                break;
    ...
    return (
        <TimerContext.Provider value={{ timers, serverOffset, loading, refreshTimers: fetchActiveTimers }}>
            {children}
